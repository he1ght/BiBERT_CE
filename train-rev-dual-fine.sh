...
TEXT=./download_prepare_reverse/data_mixed/
SAVE_DIR=./models/reverse/dual/

CUDA_VISIBLE_DEVICES=0,1,2,3 fairseq-train ${TEXT}en-de-databin/ --arch transformer_iwslt_de_en --ddp-backend no_c10d --optimizer adam --adam-betas '(0.9, 0.98)' \
...
--no-epoch-checkpoints --save-interval 1 --pretrained_model jhu-clsp/bibert-ende --use_drop_embedding 8 --concept_equalization



## Fine-Tuning

TEXT=./download_prepare_reverse/data_mixed_ft/
SAVE_DIR=./models/reverse/dual-ft/
...
--restore-file ./models/reverse/dual/checkpoint_best.pt --reset-lr-scheduler --reset-dataloader --reset-meters --concept_equalization